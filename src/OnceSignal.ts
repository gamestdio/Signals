...
export class OnceSignal {
     ...
     */
    public addOnce(listener: Function): Slot {
        return this.registerListener(listener, true);
    }

    public once (listener: Function): Slot {
        return this.addOnce(listener);
    }

    /** @inheritDoc */
    public remove(listener: Function): Slot {
        let slot: Slot = this.slots.find(listener);
        if (!slot) {
            return null;
        }
    ...
    protected registerListener(listener: Function, once: boolean = false): Slot {
        if (this.registrationPossible(listener, once)) {
            let newSlot: Slot = new Slot(listener, this, once);
    ...
    protected registrationPossible(listener: Function, once: boolean): boolean {
        ...
        let existingSlot: Slot = this.slots.find(listener);
        if (!existingSlot) {
            return true;
        }
